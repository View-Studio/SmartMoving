...
| C-26 | 클라이밍 — `climbIntoCount` (크롤-클라이밍 갭 진입 카운터) 상태 필드 및 `setShouldClimbSpeed` 연결 | — | SmartMovingClimber.java L177 | [x] → SmartMovingClientState에 climbIntoCount(int) 추가. climbIntoCount>0 시 value=HOLD_MOTION, isUp=true 강제. |
...
| C-31 | 클라이밍 — `wantClimbUp` / `wantClimbDown` 키 입력 기반 방향 제어 | — | SmartMovingClimber.java L284 | [x] → grab.isPressed() + player.input.movementForward>0→wantClimbUp, ≤0&&!isCrawling→wantClimbDown. 둘 다 false → return(vanilla). Up: FAST/MEDIUM/SLOW_UP/HOLD. Down: feetFastUp→CLIMB_DOWN, handsUp→CLIMB_DOWN, else→SINK_DOWN. |
| C-32 | 클라이밍 — `climbBackJump`, 클라이밍 중 `wallJump`, `handleCrash()` 처리 | — | SmartMovingClimber.java L312-314 | [x] → handleClimbBackJump: isClimbHolding&&jumpPending→jumpAngle=yaw+180, horiz=0.3D back, vert=0.42, climbJumpBackHead로 head/back 선택, continueWallJumping=!isHeadJumping, isClimbing=false. handleCrash: fallDistance>start→damage=(dist-start)*factor. Config에 freeClimbFallDamageStartDistance(2F)/freeClimbFallDamageFactor(2F)/climbJumpBackHead(false) 추가. |
| C-33 | 클라이밍 — SM 독자 exhaustion 시스템 (`climbExhaustion` 필드 + `climbExhaustionStart/Stop` 로직) | — | SmartMovingClimber.java L240, L341 | [x] → SmartMovingClientState에 wasClimbing/exhaustion 필드 추가. tickEssential()에서 wasClimbing=isClimbing 저장 + exhaustion=max(0, exhaustion-1F) 감소. handleClimbing()에서 exhaustion check(stop&&(wasClimbing||start)). 클라이밍 중 exhaustion+=2F. resetState()에 두 필드 초기화. |
| C-34 | 천장 클라이밍 — `grabButton` 키 `wantClimbCeiling` 조건 연결 | — | SmartMovingClimber.java L344 | [x] → wantClimbCeiling=grab.isPressed()&&!isCrawling&&!isSneaking(). ceilingClimb exhaustion 체크 추가(stop&&(isCeilingClimbing||start)). |
| C-35 | 천장 클라이밍 — `jgap` 정확한 AABB 충돌 쿼리 계산 (현재 블록 스캔 stub) | — | SmartMovingClimber.java L351, L387-403 | [x] → computeJgap: ceil(bb.maxY) 위 첫 솔리드 블록 Y - bb.maxY = 머리에서 천장까지 거리. jgap>1.2→0.12, >1.115→0.08, else→0.04. 4블록 이내 천장 없으면 2.0D 반환. |
| C-36 | 천장 클라이밍 — 수평 속도 방향 벡터 분해 및 속도 적용 | — | SmartMovingClimber.java L365 | [x] → player.input.movementForward/movementSideways + yawRad으로 motionX/Z 계산. moveFlying() 원본 공식: strafe/forward * (speed/max(dist,1)), yaw cos/sin 분해 후 setVelocity. |
| C-37 | 점프 — `Stats.JUMP` 통계 기록 (`tryJump` 내) | A-20 | SmartMovingJumper.java L179 | [x] → `if (up) player.incrementStat(Stats.JUMP)` 구현. |
| C-38 | 점프 — `Orientation.java` 이식 (`horizontalCollisionAngle` 정확한 계산) | A-23 | SmartMovingJumper.java L309-311 | [x] → calculateSeparateCollisionAngle(): 4방향 delta offset AABB 충돌 감지(world.isSpaceEmpty), X/Z swap 호출(posZ,negZ,posX,negX), NaN→(movementAngle+180)%360 폴백. Box import 추가. |
