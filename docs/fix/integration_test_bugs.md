...
- [✅ AI 완결 / 인게임 검증 대기] **BUG-26** 비행 시 땅에 닿아도 착지 안 됨 — Flying Phase F-7 (세션 43): 진짜 원인 발견 = SmartMovingConfig 의 flyCloseToGround/flyWhileOnGround default 값 **잘못 매핑** (true → 원본 false). 원본 net.smart.properties.Properties.getDefaultValue(Modified) = **false** (Properties.java L173-L174). default=false 정정 → tryLanding 가드 활성 + vanilla 자동 착지 (ClientPlayerEntity.tickMovement L1293-L1331) 그대로 작동. 세션 41/42 정정 시도들은 default 값을 가정한 가드 조작이었으나 근본 원인 = default 값 자체.
- [⏳ BUG-25 후 재평가] **BUG-27** 위아래 보면서 전진 속도 너무 느림 — F-2 결과: moveFlying 정확 1:1 → BUG-25 의 부분 증상 가능성.
- [✅ AI 완결 / 인게임 검증 대기] **BUG-28** 처음 비행 시 하늘 끝까지 날아감 — Flying Phase F-7 (세션 43 추가 분석): 세션 42 의 sm_jump 비행 가드 (가드는 유지) 외에 **추가 원인** 발견 = vanilla 1.21.1 ClientPlayerEntity.tickMovement 디컴파일 L1037-L1095 가 매 틱 `motionY += jumping*flySpeed*3 ≈ +0.15` 비행 boost 적용 (1.21.1 특화). 원본 SM 의 `motionY -= 0.15` (jump branch) 가 정확히 cancel 의도였으나 vanilla boost 와 net 0 + moveFlying +0.05 만 잔존 → 평형 ~10 m/s 무한 상승. 원본 1.7.10 EntityPlayerSP 에는 없는 1.21.1 차이. 정정: SmartMovingFlyer.handleFlying 진입 직후 vanilla boost 만큼 motionY 보정 (jumping → -flySpeed*3, sneaking → +flySpeed*3) → 원본 net -0.10 (Space hold 만으로 천천히 하강) 1:1 매칭.
