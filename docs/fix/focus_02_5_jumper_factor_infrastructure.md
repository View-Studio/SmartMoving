...
**F-6. 사용자 인게임 플레이테스트** — **세션 22: deferred (모든 포커스 완결 후 통합 테스트)**

> 사용자 결정 (2026-04-25, 세션 22): 단독 포커스 인게임 테스트 어려움 — 다른 포커스 의존
> 기능 (#3 상태 전환 / #2.6 lava border / #2.7 server sync / #1 애니메이션 등) 미완결로
> 환경 제약. 본 포커스 #2.5 의 점프 동작은 의존 포커스 완결 후 일괄 검증 가능.
>
> 따라서 F-6 은 **포커스 #2.5 완결 차단 조건이 아님**. F-1~F-5 (AI 자동 감사) 통과로 본
> 포커스는 사실상 완결. F-6 은 **모든 포커스 (#3/#4/#1/#2.6/#2.7) 완결 후 통합 인게임 테스트**
> 로 분리.

- [~] F-6. **deferred** — 통합 인게임 테스트 단계로 분리. 본 포커스 완결과 무관.
     단 아래 **체크리스트는 통합 테스트 진입 시 그대로 활용**:
...
### 세션 22 — 2026-04-25 — F-6 deferred + 포커스 #2.5 AI 완결 마킹

사용자 결정: 단독 포커스 인게임 테스트 어려움. 다른 포커스 의존 기능 미완결로 환경 제약.
   본 포커스의 점프 동작은 의존 포커스 (#3/#2.6/#2.7/#1) 완결 후 일괄 검증 가능. F-6 deferred.

진행한 작업:
1. **§3 F-6 [ ] → [~] (deferred)**: 통합 인게임 테스트 단계로 분리. 본 포커스 완결 차단 조건
   아님 명시. 체크리스트 9 시나리오는 통합 테스트 진입 시 활용.
2. **포커스 #2.5 AI 완결 마킹**: F-1~F-5 (AI 자동 감사) 통과로 본 포커스는 사실상 완결.
   F-6 은 모든 포커스 완결 후 통합 검증.
3. **다음 포커스 권고**: `playtest_fixes.md` 의 #2.5 상태를 "AI 완결 — 통합 인게임 검증 대기"
   로 갱신. 다음 포커스 진입 가능 (#2.6 lava border / #2.7 server sync / #3 상태 전환).

근사 여부: 신규 0. 코드 변경 0.

완료 전 검증 체크리스트 (세션 22 기준):
- [근거] 사용자 명시 결정 (단독 테스트 어려움)
- [근거] 다른 포커스 의존성 매핑 (§3 F-6 deferred 사유 + 본 세션 로그)
- [대응] §3 F-6 표기 갱신 + 본 포커스 AI 완결 명시
- [분기] N/A (문서 정리)
- [상수] N/A
- [타이밍] N/A
- [근사] 근사 없음. F-6 deferred 는 사용자 결정에 따른 환경 제약 — 근사 아님.
- [신규] §0/§3/§6 갱신
- [회귀] 코드 변경 0 → 회귀 0
- [빌드] BUILD SUCCESSFUL (코드 변경 없음 검증)

다음 세션 권고: **포커스 #2.5 종료 → 다음 포커스 진입**. 후보:
- **#2.6 Lava Liquid Border** (소-중, ~15 원자, 2-3 세션) — 독립 포커스, 의존 #2 완료
- **#2.7 BBox/POSE/EyeHeight 서버 sync** (Phase 2 대기, ~20 원자) — 독립
- **#3 상태 전환 조건 이상** — #2 / #2.5 / #2.6 / #2.7 완료 의존 (가장 큰 후속)
사용자 결정 대기.
