...
- `_angleJumpDoubleClickTicks`, `_wallJumpDoubleClick`, `_wallJumpDoubleClickTicks` → **처리 완료** (Config 필드+readFrom/writeTo 추가; 각도 점프 더블클릭 하드코딩 `3` → `(int)Math.ceil(angleJumpDoubleClickTicks)`로 교체. 벽 점프 더블클릭 로직은 wantWallJumping 시스템 이식에서 완전 처리.)
- `_displayExhaustionBar`, `_displayJumpChargeBar` → **처리 완료** (Config 필드 2종 default true + readFrom/writeTo 추가; SmartMovingHud 진입 조건에 원본 L405 `(displayExhaustionBar || displayJumpChargeBar)` 1:1 반영.)
- `_diveControlVertical` → **처리 완료** (Config 필드 default true + readFrom/writeTo 추가. SmartMovingSwimmer: 5-인자 moveFlying(원본 SmartMovingBase.md L77-114) 이식, 4-인자는 wrapper로 교체. isDiving 분기가 `moveFlying(motionYDiff, strafe, forward, 0.02*speed, cfg.diveControlVertical)` 호출 — 원본 SmartMovingSelf.md L476 1:1. 추가 수정: 4-인자 wrapper가 원본 정규화(total<1.0→1.0) 포함 — 기존 버전이 정규화 단계 누락했던 [오역] 도 복원.)
- `_displayExhaustionBar`/`_displayJumpChargeBar` — HUD 표시 on/off: SmartMovingHud 항상 표시
- `_sneakToggle` → **처리 완료** (리서치 보완: SmartMovingSelf.md R-09 블록 원본 L2966-L3045 + L2576 sneakContinueInput 전체 덤프. SmartMovingConfig: `sneakToggle`(Modified=false) 필드 + readFrom/writeTo. SmartMovingClientState: sneakToggled/wasSneaking/wasCrawling_st/wasClimbCrawling/sneakKeyStartPressed/sneakKeyStopPressed/prevSneakKeyPressed/jumpKeyStopPressed 8개 필드 추가. tickEssential: sneakKey+jumpKey 엣지 감지 확장(Start/Stop), 매 틱 블록 진입부에 wasSneaking/wasCrawling_st/wasClimbCrawling 저장. isSlow 계산: sneakContinueInput 경유(토글 활성 시 `sneakToggled||startPressed`, 비활성 시 `Pressed`). isSmall 계산 직후 원본 L2966-L3045 상태 전환 블록 1:1 이식(willStopCrawl/willStopCrawlStartSneak/willStopSneak/willStartSneak/willStartCrawl 5개 플래그 → sneakToggled/crawlToggled/ignoreNextStopSneakButtonPressed 갱신). resetState 에 새 필드 8개 초기화 추가. BUILD SUCCESSFUL ✓)
...
신규 발견 미구현:
- isSwim: isGenericSneaking threshold (0.005 vs 0.015) 미적용 — 시각적 영향 미미 → **처리 완료** (이전 세션)
- isDive: isLevitate/isJump 상태 미추적 — SM 비행+점프 중 수직각 abs() 미적용 (희귀 케이스) → **처리 완료** (이전 세션)
- isFeetVineClimbing + UpGrab 동시: vine "+=" vs 구현 "=" 차이 — 극히 희귀한 상태 조합 → **처리 완료** (WebFetch 로 원본 L219-L239 확인. pitch 는 `if(!isFeetVineClimbing)` 가드 블록 단독 할당, roll 은 무조건 일반 경로 실행 후 vine 블록에서 `+=` 로 누적 — 이 비대칭 구조 1:1 재구성. 리서치 파일 R-10b 섹션에 원본 코드 덤프 추가.)
...
- [잉여] sm_captureBodyYaw에 `sm.isCrawling` — 원본 rotatePlayer()에 isCrawling 단독 없음 → **처리 완료** (smActive에서 제거)
- [잉여] sm_captureBodyYaw에 `sm.isRopeSliding` — 원본에 isSliding만 있고 isRopeSliding은 별도 상태 → **처리 완료** (smActive에서 제거)
- [누락] sm_captureBodyYaw에 `sm.isClimbCrawling` 빠짐 — 원본 rotatePlayer() 조건에 포함 → **처리 완료** (smActive에 추가)
- [오역] forwardRotation 공식 — 원본 L269: `prevRotationYaw + (rotationYaw - prevRotationYaw) * f2` (player yaw 보간), 구현: `atan2(-vel.x, vel.z)` (이동 방향) → **처리 완료** (prevYaw + (getYaw - prevYaw) * tickDelta 로 교체, 이동 벡터 zero check도 제거)
...
| 2026-04-23 | `render/SmartMovingRender.md` | [오역/잉여/누락] sm_captureBodyYaw 전반 정리 — ① [오역] forwardRotation 공식: 원본 L269 `prevRotationYaw + (rotationYaw - prevRotationYaw) * f2` (player yaw 보간) vs 구현 `atan2(-vel.x, vel.z)` (이동 방향). ② [잉여] smActive 조건에 `isRopeSliding`/`isCrawling` 단독 포함(원본 rotatePlayer L271-274에 없음). ③ [누락] smActive 조건에 `isClimbCrawling` 빠짐(원본에 포함). | **처리 완료** — smActive를 원본 10개 조건(isClimbing/isClimbCrawling/isCrawlClimbing/isFlying/isSwimming/isDiving/isCeilingClimbing/isHeadJumping/isSliding/isAngleJumping) 1:1로 재정렬. forwardRotation = `prevYaw + (getYaw - prevYaw) * tickDelta`로 교체. 이동 벡터 zero check 제거(원본에 없음). 비행/각도점프/클라이밍크롤 중 body yaw 강제 정상화. BUILD SUCCESSFUL ✓
| 2026-04-23 | `config/SmartMovingOptions.md` | [미이식] 더블클릭 설정 3종 하드코딩 — `_angleJumpDoubleClickTicks`(3F, min 2F), `_wallJumpDoubleClick`(Unmodified=true), `_wallJumpDoubleClickTicks`(3F, min 2F). 각도 점프 더블클릭은 코드에 `3` 하드코딩, 벽 점프 더블클릭은 로직 전체 부재. | **처리 완료** — SmartMovingConfig: 3개 필드(float/bool/float) + readFrom/writeTo 추가(원본 키 그대로). SmartMovingClientState: 각도 점프 더블클릭 카운터 `3`→`(int)Math.ceil(cfg.angleJumpDoubleClickTicks)` 교체. BUILD SUCCESSFUL ✓ (이후 아래 `wantWallJumping` 시스템 행에서 벽 점프 더블클릭 로직 전체 이식)
| 2026-04-23 | `moving/SmartMovingSelf.md` L2863-2897 + `mapping/jump.md` L623-668 | [누락] 벽 점프 `canWallJumping`/`wallJumpCount`/`triggerWallJumping`/`wantWallJumping` 시스템 + `continueWallJumping` false 전환 로직 전체 부재. 기존 `handleWallJumping` 은 `player.horizontalCollision` 만 기준으로 바로 진입 → 더블클릭 없이 벽 충돌 즉시 발동, `isLevitating`/`isWallJumpEnabled` 조건 누락. | **처리 완료** — SmartMovingClientState: `wallJumpCount`/`triggerWallJumping`/`wantWallJumping`/`jumpKeyStartPressed`/`prevJumpKeyPressed` 5개 필드 추가. tickEssential: `jumpKeyStartPressed` 매 틱 계산(jumpKey.isPressed() 엣지 감지), `triggerWallJumping` 매 틱 리셋(원본에 명시된 리셋 위치 없어 보수적 채택). SmartMovingJumper.updateWallJumpState(신규): canWallJumping 계산(isWallJumpEnabled & !headJumping & !onGround & !climbing & !swim & !dive & !levitating & !flying), wallJumpDoubleClick 분기(true 시 타이머, false 시 jumpStartPressed 직결), wantWallJumping 자기참조 식, continueWallJumping 해제 조건. handleWallJumping 진입 조건: `player.horizontalCollision` 제거 + `!wantWallJumping` 체크로 교체(원본 L1450 1:1). sm_travel_client: handleWallJumping 직전에 updateWallJumpState 호출. BUILD SUCCESSFUL ✓
| 2026-04-23 | `render/SmartMovingRender.md` L405 + `config/SmartMovingOptions.md` L254-255 | [누락] HUD 표시 on/off 설정 `_displayExhaustionBar` / `_displayJumpChargeBar` (Unmodified=true) 및 진입 조건 `Config.enabled && (displayExhaustionBar \|\| displayJumpChargeBar)` 부재. 기존 HUD는 `cfg.enabled` 만 체크. | **처리 완료** — SmartMovingConfig: `displayExhaustionBar`/`displayJumpChargeBar` 2개 필드 default true + readFrom/writeTo 추가(원본 키 `move.gui.exhaustion.bar`/`move.gui.jump.charge.bar` 그대로). SmartMovingHud.render: 진입 조건에 `(!cfg.displayExhaustionBar && !cfg.displayJumpChargeBar) return` 추가 (원본 L405 1:1). 개별 바 렌더는 drawX 기준 유지(원본 명시 없음). BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/SmartMovingBase.md` L77-114 + `config/SmartMovingOptions.md` L277 + `moving/SmartMovingSelf.md` L476 | [누락] `moveFlying` 5-인자 버전(moveUpward, strafe, forward, speed, treeDimensional) + [오역] 4-인자 버전 정규화 단계(total<1.0→1.0) 누락 + [미이식] `_diveControlVertical`(Unmodified=true) 설정 + isDiving 분기의 3D 이동. | **처리 완료** — SmartMovingConfig: `diveControlVertical`(bool, default true) 필드 + readFrom/writeTo 추가(`move.dive.control.vertical`). SmartMovingSwimmer: 5-인자 `moveFlying(player, moveUpward, strafe, forward, speed, treeDimensional)` 원본 4단계 공식(수평 방향 + pitch 수직 보정 + 합산 + 비대칭 total sqrt(sqrt(x²+z²)+y²)) 이식. 기존 4-인자는 `moveFlying(player, 0, s, f, sp, false)` wrapper로 교체 — 정규화 단계 자동 복원. isDiving 분기: `motionY += motionYDiff` 직접 가산을 `moveFlying(motionYDiff, ..., diveControlVertical)` 호출로 교체, `fly.y` 에 moveUpward 포함되므로 중복 가산 제거. BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/SmartMovingSelf.md` R-09 L2966-L3045 + `config/SmartMovingOptions.md` L65 | [미이식] `_sneakToggle`(Modified=false) 설정 + `sneakToggled`/`wasSneaking`/`wasCrawling`/`wasClimbCrawling` 필드 + sneakKey/jumpKey StartPressed/StopPressed 엣지 감지 + 원본 R-09 상태 전환 블록(willStopCrawl/willStopCrawlStartSneak/willStopSneak/willStartSneak/willStartCrawl) 전체 부재. 기존 `isSlow` 계산은 `sneakKey.isPressed()` 단순 사용 → 토글 동작 불가능. | **처리 완료(리서치 보완 포함)** — WebFetch로 원본 소스 확보 후 SmartMovingSelf.md R-09 섹션에 L2576/L2711-2719/L2966-L3045 원본 코드 전체 덤프. SmartMovingConfig: `sneakToggle`(Modified=false) 필드 + readFrom/writeTo(`move.sneak.toggle`). SmartMovingClientState: sneakToggled/wasSneaking/wasCrawling_st/wasClimbCrawling/sneakKeyStartPressed/sneakKeyStopPressed/prevSneakKeyPressed/jumpKeyStopPressed 8개 필드 추가. tickEssential: sneakKey/jumpKey Start+Stop 엣지 매 틱 계산, 블록 진입부에 wasSneaking=isSlow/wasCrawling_st=isCrawling/wasClimbCrawling=isClimbCrawling 저장. isSlow 계산: 원본 L2576 sneakContinueInput 경유 (토글 활성 시 `sneakToggled \|\| sneakKeyStartPressed`, 비활성 시 `sneakKey.isPressed()`). isSmall 계산 직후 원본 R-09 블록 전체 1:1 이식(5개 will* 플래그 + sneakToggled/crawlToggled/ignoreNextStopSneakButtonPressed 최종 갱신). resetState 새 필드 8개 초기화. wouldWantSneak의 복잡 조건(flying/sliding/headJumping/diveDownOnSneak/swimDownOnSneak/wantCrawl/mustCrawl) 완전 이식은 후속. BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/SmartMovingSelf.md` L2577-2586 + `config/SmartMovingConfig.md` L276/L281 | [누락/미이식] `wouldWantSneak` 완전 조건(원본 L2577-2586) + `_swimDownOnSneak`/`_diveDownOnSneak`(Unmodified=true) 설정 + `isFakeShallowWaterSneaking` 필드. 기존 isSlow 는 sneakContinueInput + !sprint + !isClimbing 최소 조건만 사용. | **처리 완료(부분)** — SmartMovingConfig: `swimDownOnSneak`/`diveDownOnSneak` 2개 필드(Unmodified 기본값 true) + readFrom/writeTo(`move.swim.down.sneak`/`move.dive.down.sneak`). SmartMovingClientState: `isFakeShallowWaterSneaking` 필드 추가(기본 false, true 설정 경로 리서치 불충분 → 후속). isSlow 계산을 원본 L2577-L2586 공식으로 확장: `wouldWantSneak = !flying && !sliding && !headJumping && !(diving && diveDownOnSneak) && !(swim && swimDownOnSneak && !fakeShallow) && sneakContinueInput && (!crawlEnabled \|\| !grabPressed)`. `wouldIsSneaking = wouldWantSneak && !sprint && !isClimbing` 필드 갱신(기존 `wouldIsSneaking = isSlow;` 중복 할당 제거). `isSlow = sneakContinueInput && wouldIsSneaking`. 잔여: wantCrawl/mustCrawl 두 조건은 현재 IMPL-01 블록 내부 로컬 변수라 접근 불가 — 순서 리팩토링 후속 세션 예정. BUILD SUCCESSFUL ✓
| 2026-04-23 | `render/SmartMovingModel.md` L219-L239 | [오역] 다리 각도 `rotateAngleX(pitch)` / `rotateAngleZ(roll)` 의 비대칭 분기 구조 누락 — 원본은 pitch 를 `if(!isFeetVineClimbing)` 가드로 분기하고, roll 을 **무조건** 일반 경로 할당 후 vine 블록에서 `+=` 로 누적. 기존 구현은 단순 if/else 로 pitch+roll 둘 다 덮어쓰기 분기 → UpGrab+vine 동시 케이스에서 side-swing 값 누락. | **처리 완료(리서치 보완)** — WebFetch 로 원본 SmartMovingModel.java L219-L239 직접 확인. 리서치 파일 R-10b 섹션에 원본 코드 3단계 구조(pitch 일반 경로 가드 / roll 일반 경로 무조건 / vine 전용 덮어쓰기+누적) 명시. MixinPlayerEntityModelClient 재구성: pitch 는 `if(!isFeetVineClimbing)` 가드 블록 단독 할당, roll 은 UpGrab(fOrd≥4)이면 `feetDistSideFactor=0.5`, default 면 `0` 로 무조건 계산. vine 시 pitch `= -total` 덮어쓰기 + roll `+= ±diff` 누적. yaw 는 블록 밖에서 항상 0. 포팅 실수 패턴 3종(vine 블록 roll `=` 금지 / pitch sin-cos 분해 금지 / roll 에 vine 가드 금지) 리서치에 기록. BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/SmartMovingSelf.md` L1792-L1794 + L1797-L1802 + L2419-L2432 | [누락] `wantCrawl`/`mustCrawl` 필드 부재 + 원본 순서(L2419 wouldWantCrawl 이 L2577 wouldWantSneak 앞) 미반영 → `wouldWantSneak` 2개 조건(`!wantCrawl && !mustCrawl`) 누락 상태. 이전 세션 간소화 버전 유지 중. | **처리 완료** — SmartMovingClientState 에 `wantCrawl`/`mustCrawl` 필드 승격. tickEssential 내 isSlow 계산 **직전**에 원본 L2419-L2432 대응 pre-compute 블록 추가. `player.isSneaking()` 순환 회피를 위해 원본 L1801 처럼 raw sneakKey+sneakToggled 사용. wouldWantSneak 에 `&& !wantCrawl && !mustCrawl` 복원. IMPL-01 내부 로컬 wantCrawl/mustCrawl 재계산 제거 — 필드 재사용. resetState 에 2개 필드 초기화. BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/SmartMovingSelf.md` L2407-L2418 + L2419-L2432 + L2446-L2447 | [누락] `inputContinueCrawl` 원본 공식 및 `contextContinueCrawl` 필드/해제 경로 부재 — 이전 세션은 wantCrawl 계산에서 `sneakRaw \|\| sneakToggled` 간소화, `isCrawling` 시 `wantCrawl=true` 근사, `inputContinueCrawl`/`contextContinueCrawl` 식별 불가. | **처리 완료** — Agent WebFetch 로 원본 L2407/L2411/L2416/L2447 및 L1388 (`contextContinueCrawl=true`) 정의 확보. SmartMovingClientState: `contextContinueCrawl` 필드 승격. pre-compute 블록 재작성: ① mustCrawl(원본 L1792+L2404 flying 분기 포함). ② `inputContinueCrawl = crawlToggleEnabled ? crawlToggled : sneakPressed \|\| (!freeClimbingEnabled && grabPressed)` (원본 L2407 공식 그대로). ③ `contextContinueCrawl` 해제(원본 L2408-L2418: inputContinueCrawl/물속/mustCrawl 시 false, !isDipping 축소 근사). ④ `wouldWantCrawl` 원본 L2419-L2430 식: `!flying && ((isCrawling && (input \|\| context)) \|\| (grabStart && (toggled \|\| pressed) && onGround))`. `wantCrawl = isCrawlingEnabled && wouldWantCrawl`. ⑤ `!isCrawling` 시 `contextContinueCrawl=false` (원본 L2446-L2447). IMPL-01 진입 조건에 canCrawl 게이트(원본 L1805-L1809) 추가. `contextContinueCrawl=true` 설정 경로(fromSwimmingOrDiving)는 아래 행에서 처리. BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/SmartMovingSelf.md` L1363-L1391 (fromSwimmingOrDiving) + L100 (wasShortInWater) + L648/L1354 (호출) | [누락] `fromSwimmingOrDiving(wasShortInWater)` 미이식 — 수영/잠수 종료 후 머리 위 공간 부족 판정으로 (1) 작은 구멍 크롤링 (2) 물 아래 크롤링(contextContinueCrawl=true) 전환 로직 부재. 이 경로가 없으면 contextContinueCrawl 이 영원히 false 로만 유지되어 의미 없음. | **처리 완료** — Agent WebFetch 로 원본 L1363-L1391 + L100 + L648/L1354 호출 경로 확보. SmartMovingClientState 에 `fromSwimmingOrDiving(player, wasShortInWater)` 인스턴스 메서드 추가. 원본 조건(`wasShortInWater && !isShortInWater && !isSleeping`) 그대로, 분기: !canStandUp → isCrawling+heightOffset=-1+isDipping=false, else hasLiquidCeiling(bb.maxY+1.1 내 fluid 검사) → isCrawling+contextContinueCrawl=true+heightOffset=-1+isDipping=false. MixinLivingEntityClient.sm_travel_client: handleJumping 직후 `wasShortInWater = isSwimming_sm \|\| isDiving` 스냅샷(원본 L100), handleSwimming 반환 false 분기에 `sm.fromSwimmingOrDiving(player, wasShortInWater)` 호출(원본 handleLand L648 대응). 원본 L1354 grab 분기는 단일 호출 지점으로 통합. BUILD SUCCESSFUL ✓
| 2026-04-23 | `render/SmartMovingModel.md` L279/L308/L463/L668/L704/L740 | [오역/누락] 상태별 `bipedOuter.rotateAngleY` 분기 누락 — 원본은 상태별로 다른 값(`currentHorizontalAngle` / `horizontalAngle` threshold / `forwardRotation/RadiantToAngle` / `rotateY + horizontalAngle`)을 사용하지만 1.21.1 `sm_captureBodyYaw` 는 Swim/Dive 제외 전부 forwardRotation(player yaw 보간)으로 통합 처리. 특히 isHeadJumping/isCrawling/isRopeSliding 에서 이동 방향(currentHorizontalAngle) 대신 카메라 방향(player yaw) 적용되어 렌더 방향 [오역]. | **처리 완료(부분)** — sm_captureBodyYaw 를 원본 if-else 체인 우선순위로 재구성: ① isSwimming_sm/isDiving(threshold 0.005/0.015 isSlow 기반) — 기존 유지. ② isFlying(threshold 0.05F horizontalDistance 기반) — 신규 추가(원본 L704). ③ isHeadJumping/isCrawling/isRopeSliding — currentHorizontalAngle(라디안→도) 신규 추가(원본 L279/L668/L740). ④ 나머지(isClimb/ClimbCrawling/CeilingClimb/Sliding/AngleJumping) — forwardRotation 유지(isClimb 원본 L308 forwardRotation/RadiantToAngle 과 등가). 잔여: isCeilingClimb 의 `rotateY + horizontalAngle`(L476) — 아래 행에서 완성. BUILD SUCCESSFUL ✓
| 2026-04-23 | `render/SmartMovingModel.md` L463-L476 (isCeilingClimb rotateAngleY) | [누락] `isCeilingClimbing` 시 `bipedOuter.rotateAngleY = rotateY + horizontalAngle` 미이식 — 이전 세션은 CeilingClimb 을 forwardRotation 일반 경로로 유지. | **처리 완료** — sm_captureBodyYaw isSwim 앞에 isCeilingClimbing 전용 분기 추가. 원본 L463+L475-L476 공식 1:1 이식: `distance = totalHorizontalDistance * 0.7F`, `walkFactor = Factor(currentHorizontalSpeed, 0, 0.12951545F)` (Factor 인라인: x≥border→1, x≤0→0, else x/border), `horizontalAngle = horizontalDistance < 0.015F ? currentCameraAngle : currentHorizontalAngle`, `rotateY = cos(distance) * 0.44F * walkFactor`, `smBodyYawOverride = toDegrees(rotateY + horizontalAngle)`. SmartStatistics 의 `totalHorizontalDistance` / `currentHorizontalSpeed` / `horizontalDistance` / `currentCameraAngle` / `currentHorizontalAngle` 기존 필드 재사용. BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/SmartMovingSelf.md` L226-L246 (isFakeShallowWaterSneaking=true 설정) | [누락] `isFakeShallowWaterSneaking = true` 설정 경로 미이식 — 필드만 이전 세션 추가됐을 뿐 실제 true 로 되는 로직 부재 → wouldWantSneak 의 `!(swim && swimDownOnSneak && !fakeShallow)` 조건이 항상 fakeShallow=false 로 평가됨. | **처리 완료(간소 이식)** — SmartMovingSwimmer.handleSwimming 시그니처에 `wasSwimming, wasDiving` 파라미터 추가(원본 L97-L99). 원본 L226-L246 블록 1:1 이식: `couldStandUp = dippingDepth>=0 && dippingDepth<=1.5F`(수심 기반 근사, 원본 playerSwimWaterBorder/minPlayerSwimWaterDepth 계산의 간소), `wantShallowSwim = couldStandUp && (wasSwimming \|\| wasDiving)`, 조건 `wasSwimming && wantShallowSwim && sneak && swimDownOnSneak` 일 때 `isFakeShallowWaterSneaking = true`, 아니면 false. 매 틱 상태 갱신. MixinLivingEntityClient.sm_travel_client: 기존 `wasShortInWater` 스냅샷을 `wasSwimming`/`wasDiving` 으로 분해하여 handleSwimming 에 전달. BUILD SUCCESSFUL ✓
| 2026-04-23 | `moving/Orientation.md` L361-L373 (isTunnelAhead) + `moving/SmartMovingSelf.md` L234-L240 (getClimbingOrientations 정제) | [누락] 얕은 물 스니크 정제 — 원본은 `wantShallowSwim` 계산 후 `getClimbingOrientations(sp, true, true)` 반환 방향들에 대해 `isTunnelAhead(world, i, j, k)` 가 하나라도 true 이면 `wantShallowSwim = false` 로 차단. 이전 세션 isFakeShallowWaterSneaking 간소 이식에서 이 정제 검사 생략됨. | **처리 완료(간소 이식)** — SmartMovingSwimmer 에 `isTunnelAhead(world, i, j, k, dir)` private 헬퍼 추가(원본 Orientation.md L361-L373 1:1): `(i+di, j+1, k+dk)` collisionShape.isEmpty() && `(i+di, j+2, k+dk)` collisionShape 비-empty. `getClimbingOrientations(ortho=true, diag=true)` 는 `Direction.Type.HORIZONTAL` 4방향으로 근사(대각 4방향 생략). isFakeShallowWaterSneaking 설정 앞에 정제 루프: 4방향 중 하나라도 터널이면 wantShallowSwim=false 로 차단. 원본 isFullEmpty/isSolid 의 간판/압력판/AS로프 예외는 간소 버전에서 생략. BUILD SUCCESSFUL ✓
...
| 2026-04-23 | `config/SmartMovingServerOptions.md` | [오역] `processSpeedChangePacket` 서버측 config 적용 누락 — 원본: `options.changeSpeed(diff, player)` → `config.changeSpeed(diff) + saveToOptionsFile + logSpeedState`. 1.21.1: 응답만 전송. | **처리 완료** (2단계 수정) — ① 최초: `INSTANCE.changeSpeed(전역) + save() + logSpeedState` 추가. ② 이후 아래 `changeSingleSpeed` 이식(행) 단계에서 원본 흐름 재정정(관리자 경로 `changeSpeed` vs 일반 유저 경로 `changeSingleSpeed` 분리): `changeSpeed(전역)` → `changeSingleSpeed(개인)` 로 교체, `logSpeedState` 호출 제거(원본 `setPlayerSpeedExponent`는 로그 없음). 관리자 자발적 전역 변경 + 로그 경로는 서버 커맨드 이식 시 재추가 예정. BUILD SUCCESSFUL ✓ |
| 2026-04-23 | `config/SmartMovingServerOptions.md` | [미이식] `toggle(player)` — 서버 관리자가 config key를 순환하는 명령. SmartMovingConfig의 config key 시스템(`_configKeyName`/`currentKey`/`getNextKey`/`setCurrentKey`/`setKeys`) + 채팅 커맨드 기반. | **처리 완료** — 원본 toggle()/SmartMovingProperties.toggle() 확인 결과 단일 config key 시스템(1.21.1 의 단일 "default" key 단순 on/off 토글)이 원본 `toggler==0 ↔ -1` 순환과 기능 동등. SmartMovingServer.adminToggleConfig(player) 신규: `INSTANCE.toggle() + save() + logConfigState(reconfig=true) + broadcastConfig(server)`. broadcastConfig: 접속 중 initialized 모든 플레이어에게 ConfigContentPayload 재전송(플레이어별 username 기반 toArray 사용). `/smoving config toggle` 커맨드(op 권한 2) 등록 — SmartMoving.onInitialize 에 CommandRegistrationCallback 추가. BUILD SUCCESSFUL ✓ |
| 2026-04-23 | `config/SmartMovingServerOptions.md` | [미이식] `changeSpeed(diff, player)` 서버 관리자 자발적 — `processSpeedChangePacket` 의 클라이언트 요청 수락 경로와 별개로, 서버측에서 커맨드로 호출되는 자발적 변경. | **처리 완료** — SmartMovingServer.adminChangeSpeed(player, diff) 신규: `INSTANCE.changeSpeed(diff) + save() + logSpeedState + broadcastConfig`. 원본 L73-L78 1:1. `/smoving speed <diff>` 커맨드(op 권한 2, IntegerArgumentType -100~100) 등록. processSpeedChangePacket 경로(클라 요청 수락=changeSingleSpeed 개인 변경)와 별개 — 이쪽은 전역 변경. BUILD SUCCESSFUL ✓ |
...
| 2026-04-23 | `config/SmartMovingProperties.md` + `config/SmartMovingOptions.md` + `config/SmartMovingConfig.md` | [오역/누락] **4상태 토글 복원** — 이전 감사에서 "단일 key=on/off 가 원본 toggler 0↔-1 순환과 기능 동등" 으로 오판하고 L1092 에서 단순화 이식. 실제 원본은 gameType 별 길이 3 배열(`{"e","m","h"}`)로 `disabled→easy→medium→hard→disabled` 4상태 순환. 단일 on/off 토글은 Creative(`{"c"}`) 길이 1 케이스에만 등가. | **처리 완료** — 포커스 #5 (`docs/fix/focus_05_config_toggle.md`) 로 분리 진행. **B-1~4** 필드(configKeys/toggler/configKeyName + 게임타입별 6필드). **C-1~7** 원본 1:1 메서드 (toggle/setKeys/updateToggler/getCurrentKey/getKey+getNextKey/hasKey/setCurrentKey). **D-1** `SmartMovingServer.logConfigState` 4상태 분기 복원(default/with key/named). **E-1~3** 클라이언트 채팅 피드백 4갈래(disabled/enabled/named/unnamed) + `en_US.lang` 원본 텍스트 확보 + 번역 키 정정. **F-1~3** `initializeForGameIfNeccessary`/`resetForNewGame` 이식 + `MinecraftClient.interactionManager.getCurrentGameMode().getId()` 기반 gameType 판정 + `SmartMovingClientState.tickEssential` 매 tick 폴링 호출. 잔여: `toggler` 영속화 + 서버→클라 동기화 + `_configChatInit` 분기 — focus_05 §17 후속 (`focus_08_toggle_persistence.md` 후보). BUILD SUCCESSFUL ✓ |
| 2026-04-24 | `config/SmartMovingConfig.md` + `config/SmartMovingClientConfig.md` + `moving/SmartMovingSelf.md` | [오역/누락] **Easy 1:1 허기/소진 공식 이식** — 이전 감사에서 4상태 토글을 라벨만 복원하고 "enabled=원본 Easy 1:1" 은 dead field 로 방치. 사용자 지적(세션 13)으로 Easy 프리셋 완전 이식 결정. H-2 매트릭스 결과 20개 중 19개 dead 확인 후 범위 축소(세션 15): "Easy 에서 실제 코드 경로 진입하는 것만" 이식. | **처리 완료** — 포커스 #5 H 섹션 (`docs/fix/focus_05_config_toggle.md` §10 H-0~H-14). **H-3** factor 1단계 Config 13필드 (baseExhautionLossFactor Easy 1.2F / baseHungerGainFactor Easy 0.8F + sprinting/running/sneaking/standing/walking/fall exhaustion+hunger 쌍). **H-4** factor 2단계 14필드 (climbing/crawling/ceilClimbing/swimming/diving/dipping/normal 쌍 — Exhaustion 정타 / ceilClimbing 축약 / L589-L591 normal 중복 유지). **H-5** alwaysHungerGain(0F) + exhaustionLossHungerFactor(Easy 0.02F). **H-6** speedUser 정정 true→false (Creative 팩토리 Easy 반환값). **H-7** `Config.getFactor(hunger, 14상태)` 메서드 이식 (원본 L554-L595 1:1, airBorne+hunger 0F 하드코딩 + L589-L591 normal 중복 완전 재현). **H-8** ClientState 에 hungerIncrease/lastHungerIncrease 필드 + resetState 초기화. **H-9** `ClientState.handleExhaustion(player)` 축소판 이식 (Easy 실행 경로만 — 이동거리/isStanding/isStill/isRunning 파생 + getFactor(true) 허기 + getFactor(false) 소진 + 허기-소진 연동). **H-10** tickEssential 말미에 호출 삽입 + 기존 `exhaustion -= 1.0F` 중복 제거. **H-11** `handleExhaustion` 말미에 원본 L911-L915 허기 패킷 송신 이식 (변화 감지 → ClientPlayNetworking.send(HungerChangePayload)). Easy 1:1 원칙: Exhaution/Exhaustion 오타 혼재 원본 그대로, `_ceilClimbing*` 축약, airBorne hunger 필드 없음(0F 하드코딩), up() 동적 하한은 단순 default 근사, 난이도 체인은 base 2개에만 적용. 배제 (Easy 진입 안 함): 14종 점프 피로 / 클라이밍·천장·스프린트 피로 축적 / 라바 수영 / Creative levitate / getMaxExhaustion 순회. 잔여 (§17 후속): 서버 허기 연동 3디테일(sm.hunger 리셋 / withinOnLivingUpdate / 조건) — `focus_11_server_hunger_sync.md` 후보. BUILD SUCCESSFUL ✓ (./gradlew clean build H-13) |
| 2026-04-24 | `config/SmartMovingConfig.md` | [수정] **포커스 #5 재개 — H-15~H-21 (세션 22-23)**. 인게임 검증에서 2건 문제 발견 후 재개: (1) configToggle 4상태 스위칭 잔존 — F 섹션 `initializeForGameIfNeccessary` 매 tick 호출이 `setKeys({"e","m","h"})` 덮어씀 (§7.2 "잉여 무해" 판단 오류). (2) 허기 폭주 — 5초 내 ~12 food 소비. 클라 hungerIncrease 누적값 송신 + 서버 매 틱 `addExhaustion(누적값)` 중복 적용. | **처리 완료** — **H-15** `SmartMovingClientState.tickEssential` 의 `initializeForGameIfNeccessary` 호출 제거 → 2상태 토글 강제 복원. **H-16** 허기 delta 전송 전환: 클라 `send(hungerIncrease - lastHungerIncrease)` / 서버 `sm.hunger += payload` 누적 (MixinServerPlayerEntity TAIL 기존 `addExhaustion + 0F 리셋` 유지). **H-17** configToggle 재전환 NPE 긴급 수정 (ImmutableMap null key 조회): 임시 `currentKey == null ? ""` 가드 추가. **H-18** 잉여 정리 (활성 경로): 클라 4갈래→2갈래 / 서버 3갈래→1갈래 / `en_us.json` `.named`/`.unnamed` 키 + `configKeyName` Map 삭제 (H-17 가드 함께 제거). **H-19** 메서드/상수 7개 삭제: `getCurrentKey`/`getKey`/`getNextKey`/`hasKey`/`setCurrentKey` + `CONFIG_KEY_ENABLED`/`CONFIG_KEY_DISABLED`. **H-20** gameType 시스템 전체 삭제: 메서드 2(`initializeForGameIfNeccessary`/`resetForNewGame`) + 필드 7(gameType + 게임타입별 6) + 상수 4(GAME_TYPE_*) + readFrom/writeTo 6키 + 헬퍼 2(`getCsvArray`/`csvJoin`). 세이브 파일 호환: 기존 키 무시. **H-21** clean build 재통과 + 회귀 감사 재수행 (grep 6건 전부 통과, 회귀 0건). 원본 본체는 `research/original/smartmoving/config/SmartMovingProperties.md` L99-L156 에 보존 — Medium/Hard 프리셋 복원(`focus_09_difficulty_presets.md`) 시 재도입. 서버 허기 동기화 3디테일은 여전히 `focus_11_server_hunger_sync.md` 후속 (delta 우회로 체감 근사). BUILD SUCCESSFUL ✓ |
| 2026-04-24 | `moving/SmartMovingSelf.md` L2326 + L119 + L476-L561 + L780/L822/L843/L893/L1522 | [오역/누락] **포커스 #6 — increase/decrease 작동 안 됨**. 사용자 인게임 테스트: speedIncrease/Decrease 키 누르면 채팅 텍스트만 뜨고 실제 이동 속도 안 바뀜. 원인: `getUserSpeedFactor()` 수학 공식은 이식됐으나 이동 처리 블록(Land/Swim/Climb) 에 곱셈 누락. 추가로 `_speedUser = Creative("move.speed.user")` 팩토리의 Creative 전용 감지가 포커스 #5 H-20(gameType 시스템 삭제) 때 사라짐. | **처리 완료** (focus_06_speed_change.md §10 A~D). **A** 호출처 감사 — 원본 14곳 vs 1.21.1 3곳, 누락 2건 + 확인 4건 매핑. **B-1** Mover 헬퍼 정비 (getConfigSpeedFactor / getPotionSpeedFactor 에 cfg.enabled 가드 + getCombinedSpeedFactor 단일 헬퍼 신설 — 원본 Self L149-L162 1:1). **B-2** Land 이동 이식 (MixinLivingEntityClient.sm_getMovementSpeed HEAD inject, vanilla travel 경로에 Mover.getConfigSpeedFactor 주입). **B-3** Swim/Dive 이식 (Swimmer L190 speedFactor 에 getCombinedSpeedFactor 곱). **B-4** Climb 3갈래 점검 — 하강 클램프 -0.15D 고정 → User 배율 반영, Free climb combinedFactor=1.0D 하드코딩 → 실제 factor 전달 수정, Smart 모드는 기존 이식 확인. **B-6** Creative 전용 게이트 신설 (원본 _speedUser Creative 팩토리 1:1) — Mover.isCreative 헬퍼 + getConfigSpeedFactor player 파라미터 + ClientState speedIncrease/Decrease 키 블록 Creative 가드 + Server processSpeedChangePacket Creative 체크. **C-1/C-2** 서버-클라 동기화 정합 (changeSingleSpeed 개인 맵 / toArray username 치환 / SERVER_CONFIG 덮어쓰기 일관성). **D-1** clean build ✓. **D-3** 회귀 감사 — 기본 (!speedUser or !Creative) 상태 vanilla 동작 불변, 회귀 0건. **D-2** 사용자 인게임 테스트 속도 조정 정상 작동 확인. **B-5** 점프 maxHorizontalMotion — 1.21.1 에 getMaxHorizontalMotion 시스템 전체 부재 → 범위 초과, `focus_12_max_horizontal_motion.md` 후속. **배율 미세 차이** (스프린트 1.3 vs 1.5 등) — getNonSlowInputSpeedFactor / getSlowInputSpeedFactor land 경로 미반영 → `focus_13_movement_factor_system.md` 후속. BUILD SUCCESSFUL ✓ |
| 2026-04-24 | `moving/SmartMovingSelf.md` L232-L474 + L2395-L2874 + L2966-L3045 + R-09 + R-10~R-15 | [오역/누락] **포커스 #2 — 스마트무빙 상태 이상**. 인게임 검증: 13 상태 플래그 (isCrawling / isDipping / isSwimming_sm / isDiving / isClimbing / isCrawlClimbing / isCeilingClimbing / isClimbCrawling / isSliding / isHeadJumping / isFast / isSlow / contextContinueCrawl 등) 잘못된 시점에 true/false. 소비처 #1 애니메이션 / #3 전환조건 / #4 키커맨드 연쇄 오동작. 원본 grep + 1:1 코드 감사 방식 전환 (세션 29). | **처리 완료** (focus_02_state_issues.md §10 A~C / 세션 29-86). **A 단계** A-0~A-7 감사 (Agent WebFetch 로 원본 R-10~R-15 전수 덤프, 불일치 70건 확정). **B Phase 1** 필드 이식 일괄 (등반 9 + 수중 4 + 전환 4 + 크롤 3 = 20+ 필드). **B Phase 2** 54 원자 완료 + 4 원자 별도 포커스 분리. 핵심 원자: **B-33 + B-44b** (isCrawling IMPL-01 이원화 → 원본 L2441-L2442 매 틱 공식 `isCrawling = canCrawl && (wantCrawl \|\| mustCrawl)`, wasCrawling 공식 직전 저장, 세션 84). **B-16 + B-16a/b/c** wantClimbHolding 3-OR + wouldWantClimb 4-OR 완전 이식 + Climber.isFacedToLadder/isFacedToSolidVine + freeClimbAutoLadder/Vine Config 필드 (세션 68/69). **B-17a/b1/b2 + B-17b2-pre** isCrawlClimbing 메인 공식 + canStandUp 분기 + else if (wasCrawlClimbing) 3분기 + wantClimbUp/Down 필드 승격 (세션 47/48/72/73). **B-18 + B-18-pre + B-18b** isClimbCrawling 공식 + climbIntoCount 카운터 + 진입/해제 엣지 (세션 81/83). **B-1c~f + B-3a/b + B-2** isFast 6갈래 OR + wantSprint 6조건 OR + isSlow Config 게이트 정정 (세션 49-52). **B-25 + B-26** isSliding 직접 진입 6-AND + 부수 동작. **B-6/B-8/B-10d/B-12/B-13** Swimmer.updateSwimState 5건 (3-OR/Config 게이트/isLevitating 공식/ticks 조건/isSliding). **B-14 + B-21** resetClimbing() 매 틱 리셋. **B-30/B-34/B-35/B-36/B-37/B-38/B-41** 각 전환 분기. **B-43 + B-44a/b/c** R-09 종료부 저장 + 저장 시점 정밀 조정 (wasSneaking/wasCrawling/wasClimbCrawling 전부 공식 직전 이동). **B-46** grab.wasPressed() 다중 호출 해소 (§16 세션 65 발견). **B-20** Standard Base Climb isClimbing 오염 해소. **B-39** landMotionPost 3분기 구조 복원 (근사). §7 근사 지점 7건 등록 (focus_14 AABB 정밀 개선으로 일괄 해소 경로). **잔여 별도 포커스 분리**: B-7 → focus_15 (liquid climbing), B-9+B-11 → focus_16 (swim classification), B-19 → focus_17 (climb orientation), B-42 → focus_14 (AABB 정밀). **C-1/C-2** clean build + 회귀 방지 감사 (§14 4 체크포인트 + 공통 B/C/D 전수 통과). 세션 29-86 누적 54 원자 완료 (포커스 #2 "13 상태 플래그 값 자체의 정확성" 핵심 목적 달성). BUILD SUCCESSFUL ✓ |
