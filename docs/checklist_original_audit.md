...
- `_angleJumpDoubleClickTicks`, `_wallJumpDoubleClick`, `_wallJumpDoubleClickTicks` → **처리 완료** (Config 필드+readFrom/writeTo 추가; 각도 점프 더블클릭 하드코딩 `3` → `(int)Math.ceil(angleJumpDoubleClickTicks)`로 교체. 벽 점프 더블클릭 로직 자체는 triggerWallJumping/wantWallJumping 시스템 전체 이식 필요 → 후속)
