...
| 2026-04-23 | `moving/SmartMovingSelf.md` L1792-L1794 + L1797-L1802 + L2419-L2432 | [누락] `wantCrawl`/`mustCrawl` 필드 부재 + 원본 순서(L2419 wouldWantCrawl 이 L2577 wouldWantSneak 앞) 미반영 → `wouldWantSneak` 2개 조건(`!wantCrawl && !mustCrawl`) 누락 상태. 이전 세션 간소화 버전 유지 중. | **처리 완료** — SmartMovingClientState 에 `wantCrawl`/`mustCrawl` 필드 승격. tickEssential 내 isSlow 계산 **직전**에 원본 L2419-L2432 대응 pre-compute 블록 추가: !isCrawling 시 `grabJustPressed && (sneakRaw \|\| sneakToggled) && onGround && !flying && !swim && !dive && !dipping && !climbing && !crawlClimbing && !ceilingClimbing && !sliding && !headJumping`, isCrawling 중이면 유지 근사 `wantCrawl=true`. `mustCrawl = !canStandUp && !swim && !dive && (!dipping \|\| dippingDepth<0.65F)` (isCrawling 중이면 swim/dive 게이트 제외). `player.isSneaking()` 순환 회피를 위해 원본 L1801 처럼 raw sneakKey+sneakToggled 사용. wouldWantSneak 에 `&& !wantCrawl && !mustCrawl` 복원. IMPL-01 내부 로컬 wantCrawl/mustCrawl 재계산 제거 — 필드 재사용. resetState 에 2개 필드 초기화. BUILD SUCCESSFUL ✓
