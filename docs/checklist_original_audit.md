...
| 2026-04-23 | `config/SmartMovingOptions.md` | [미이식] 더블클릭 설정 3종 하드코딩 — `_angleJumpDoubleClickTicks`(3F, min 2F), `_wallJumpDoubleClick`(Unmodified=true), `_wallJumpDoubleClickTicks`(3F, min 2F). 각도 점프 더블클릭은 코드에 `3` 하드코딩, 벽 점프 더블클릭은 로직 전체 부재. | **처리 완료(부분)** — SmartMovingConfig: 3개 필드(float/bool/float) + readFrom/writeTo 추가(`move.jump.angle.double.click.ticks` 등 원본 키 그대로). SmartMovingClientState: 각도 점프 더블클릭 카운터 `3`→`(int)Math.ceil(cfg.angleJumpDoubleClickTicks)` 교체. 벽 점프 더블클릭 로직은 triggerWallJumping/wantWallJumping/continueWallJumping 전체 시스템 이식이 선행(후속). BUILD SUCCESSFUL ✓
